    ...
    for (uint i = 1; i < plsize; i++) {
      if ((msg->points[i].line < N_SCANS) &&
        ...
        if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) ||
          ...
          pl_buff[msg->points[i].line].push_back(std::move(pl_full[i]));
        }
      }
    }
    static int count = 0;
...
void Preprocess::velodyne32_handler(
    const sensor_msgs::msg::PointCloud2::UniquePtr& msg) {
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
    ...
    for (uint i = 1; i < plsize; i++) {
      if ((pl_orig.points[i].line < N_SCANS) &&
        ...
        pl_full[i].curvature =
            ...
            (time_stamp *
             1000.f);  // use curvature as time of each laser points

        bool is_new = false;
        if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) ||
            (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) ||
            (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7)) {
          pl_buff[pl_orig.points[i].line].push_back(std::move(pl_full[i]));
        }
      }
    }
    static int count = 0;
