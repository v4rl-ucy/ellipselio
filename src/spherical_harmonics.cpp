...
SphericalHarmonicsProjector::SphericalHarmonicsProjector(int l_max,
                                                         bool use_gpu)
    : l_max_(l_max), use_gpu_(use_gpu), n_coeffs_((l_max + 1) * (l_max + 1)) {}
...
// Stub for GPU implementation
void SphericalHarmonicsProjector::computeOnGPU(
    const std::vector<Vec3f>& directions, const std::vector<Vec3f>& colors,
    SHCoeffs& out_coeffs) {
...
Eigen::Vector3f SphericalHarmonicsProjector::evaluateColorFromDirection(
  ...
  float r = std::max(dir.norm(), 1e-8f);
  ...
  for (int l = 0; l <= l_max_; ++l)
    for (int m = -l; m <= l; ++m)
      Y(idx++) = val(SH(l, m, theta, phi));  // val() ensures type compatibility
